...
const totalSum = (input) => {
    // Solution code here...
    return input.reduce((accumallator, value, index) => {
        accumallator += input[index].reduce((acc, val) => {
            acc += val;
            return acc;
        }, 0)
        return accumallator;
...
const divisibleByFiveTwoToThePower = (input) => {
    // Solution code here...
    let div = input.map((value, index) => {
        return input[index].filter(val => {
            return typeof val === 'number' && val % 5 === 0;
        })
    })
    return div.map((value, index) => {
        return div[index].map(val => {
...
let findMaleAndFemale = (data) => {
    // Solution code here...
    return data.reduce((accumallator, value) => {
        if (value.gender === 'male' || value.gender === 'female') {
          accumallator.push(value.name);
        }
        return accumallator;
      }, []).join(' and ');
...
let findShortest = (data) => {
    // Solution code here...
    return data.reduce((accumallator, value) => {
        if (parseInt(value.height) < parseInt(accumallator.height)) {
          accumallator = value;
        }
        return accumallator;
      }, data[0]).name;
